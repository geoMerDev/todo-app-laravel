<div>
    @livewire('todo-category.update')
    @livewire('todo-category.delete')
    <div class="todo overflow-x-auto">
      

        <div class=" p-5 flex justify-between gap-4">
            @livewire('todo-category.create')
            <div class="w-full">
                ...
                <div class="relative">
                    ...
                    <input type="search" id="default-search" wire:model="search" wire:keydown.enter="searchInDb"
                        ...
                        placeholder="Search  Todos Categories" required>
        ...
        @if ($TodosCategories->count())
                ...
                <tbody>
                    @foreach ($TodosCategories as $TodoCategory)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <td class="px-6 py-4">
                                {{ $TodoCategory->createdBy ? $TodoCategory->createdBy->name : 'Unknown' }}

                            </td>
                            <td class="px-6 py-4">
                                <button type="button" title="editar"
                                    wire:click="$dispatch('todoCategory-edit', { todoCategoryEdit: {{ $TodoCategory }} })"
                            ...
                            <td class="px-6 py-4">
                                <button type="button" title="eliminar"
                                    wire:click="$dispatch('todoCategory-delete', { todoCategoryDelete: {{ $TodoCategory }} })"
            ...
            @if ($TodosCategories->hasPages())
                <div class="m-5">
                    {{ $TodosCategories->links() }}
                </div>
            @endif
        @else
